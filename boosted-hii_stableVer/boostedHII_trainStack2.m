function stacking = boostedHII_trainStack2(X,y,weak_learners,boostingOpts)

stackingOpts = boostingOpts.stackingOpts;
if ~stackingOpts.use
    stacking = [];
    return;
end
...
clos = zeros(n,p);
for j=1:p
    clos(:,j) = boostedHII_evaluateWeakLearner(X,weak_learners,j,[],boostingOpts);
end
clo = sum(clos,2);
